...
public class RoyaltyReportsDAO {
	...
	*/
	...
	public Collection<Report> getUserFavReports(HttpServletRequest req) {
		...
		String sql = "select rl.report_id, rl.report_group_name, rl.report_name, rl.cat_id from user_fav_reports ufa JOIN report_list rl ON (ufa.report_id = rl.report_id AND ufa.user_id=?) order by rl.report_name asc";
	...
	private String getFavRepPath(final int catId) {
		...
		String path = "";
		String sql = "select parent_id from report_cat_tree where (cat_id  = ?)";
		...
		Object o = this.jdbcTemplate.queryForObject("select name from report_cat_tree where (cat_id  = ?)",
	...
	public Report isReportInFavs(final Report report, HttpServletRequest req) {
		...
		String sql = "select count(*) from user_fav_reports where (user_id  = ? AND report_id=?)";
	...
	public Collection<Report> addReportToFav(final Report report, HttpServletRequest req) {
		...
		String sql = "INSERT INTO user_fav_reports " + "(user_id, report_id) VALUES (?, ?)";
	...
	public Collection<Report> removeReportFromFav(final Report report, HttpServletRequest req) {
		...
		this.jdbcTemplate.update("DELETE from user_fav_reports where user_id= ? AND report_id = ?",
	...
	public Collection<Report> getReportList(final ReportTreeNode parent, final String reportFilter,
		...
		String sqlrole = "select role_id from report_users where user_id=?";
		...
		if (roleId == 1) {
			...
			sql = "select report_id,cat_id,report_name from report_list where cat_id =?";
			...
			sql = "select report_id,cat_id,report_name from report_list where cat_id =? AND report_id IN (select REPORT_ID from report_list_user_roles where role_id=? )";
	...
	public Report getReport(int reportId, final String searchKeyword, final FetchLevel level) {
		try {
			...
			Report report = (Report) this.jdbcTemplate.queryForObject(
					"select * from report_list where report_id = ?", new Object[] { new Integer(reportId) },
					new RowMapper() {
						@Override
						public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
							Report report = new Report();
	...
	public Collection<ReportVersion> getReportVersionList(int reportId, final String searchKeyword,
		...
		try {
			Collection<ReportVersion> reportVersions = this.jdbcTemplate.query(
					"select report_s_no, report_id, number_of_pages, TO_CHAR(create_date, 'yyyy-mm-dd hh24:mi:ss') AS create_date, pdf_file_location from report_version where report_id = ? order by create_date desc",
	...
	public ReportVersion getReportVersion(final int sno, final FetchLevel level) {
		...
		ReportVersion reportVersion = (ReportVersion) this.jdbcTemplate.queryForObject(
				"select report_s_no, report_id, number_of_pages,TO_CHAR(create_date, 'yyyy-mm-dd hh24:mi:ss') AS create_date,pdf_file_location from report_version where report_s_no = ?",
	...
	public Collection<ReportPage> getReportPagesList(long sno, String searchKeyword, final FetchLevel level) {
		...
		String sql = "select * from report_pages where report_s_no = ?";
		Object[] fillers;
	...
	public ReportPage getReportPage(int sno, int page_no) {
		...
		ReportPage reportPage = this.jdbcTemplate.queryForObject(
				"select * from report_pages where report_s_no = ? and page_no = ?",
	...
	public int getReportPageCount(int sno) {
		return this.jdbcTemplate.queryForObject(
				"select count(*) from 	 report_pages where report_s_no = ?", new Object[] { sno },
				Integer.class);

	}

	public int getReportTreeMaxLevels() throws Exception {
		return this.jdbcTemplate.queryForObject("select max(node_level) from  report_cat_tree",
				Integer.class);
	...
	public ReportTreeNode getReportTreeNode(ReportTreeNode parent, final int level, final String path,
		...
		Collection<ReportTreeNode> childNodes = this.jdbcTemplate.query(
				"select cat_id,name,path from report_cat_tree where node_level = ? and path like  ?",
	...
	public static void main(String[] args) {
		...
		log.info("ds " + ds);
		//rrDao.setDataSource(ds);//this is database connection testing via main class
		...
		try {
			...
			if (froleId == 1) {
				...
				innersql = "select rp.report_id,"
						+ "(SELECT rv.NUMBER_OF_PAGES from REPORT_VERSION rv where rv.REPORT_S_NO=rp.report_s_no) NUM_OF_PAGES,(SELECT rvt.create_date from REPORT_VERSION rvt where rvt.REPORT_S_NO=rp.report_s_no) CRE_DATE,(SELECT SYS_CONNECT_BY_PATH(rct.name, '>') FROM report_cat_tree rct  where rct.cat_id IN (select cat_id from report_list rl where (rl.report_id=rp.report_id)) start with parent_id is null CONNECT BY PRIOR cat_id = parent_id) path,rp.report_s_no, COUNT(DISTINCT rp.page_no) occurances from report_pages rp where CONTAINS(rp.page_data,?,1)>0 group by rp.report_id,rp.report_s_no";
				...
				innersql = "select rp.report_id,(SELECT rv.NUMBER_OF_PAGES from REPORT_VERSION rv where rv.REPORT_S_NO=rp.report_s_no) NUM_OF_PAGES,(SELECT rvt.create_date from REPORT_VERSION rvt where rvt.REPORT_S_NO=rp.report_s_no) CRE_DATE,(SELECT SYS_CONNECT_BY_PATH(rct.name, '>') FROM report_cat_tree rct  where rct.cat_id IN (select cat_id from report_list rl where (rl.report_id=rp.report_id)) start with parent_id is null CONNECT BY PRIOR cat_id = parent_id) path,rp.report_s_no, COUNT(DISTINCT rp.page_no) occurances from report_pages rp where CONTAINS(rp.page_data,?,1)>0 AND rp.report_id IN (select REPORT_ID from report_list_user_roles where role_id=? ) group by rp.report_id,rp.report_s_no";
				sql = "select * from (SELECT  tmp.*, rownum as row_no FROM (" + innersql
						+ ") tmp ) where row_no between ? AND ?";
	...
	public boolean validateUser(String user, HttpServletRequest request) throws SQLException {
		...
		log.error("user:::" + user);
		String sql = "SELECT count(USER_ID) FROM REPORT_USERS where USER_ID =?";
		int test1 = 5;
		...
		if (test1 > 0) {
			sql = "SELECT USER_ID, FIRST_NAME, LAST_NAME FROM REPORT_USERS where USER_ID =?";
